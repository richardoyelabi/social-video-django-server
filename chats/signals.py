...
def check_media_ownership(sender, instance, **kwargs):
    if instance.media_item:
        if instance.media_item.uploader != instance.user:
            raise MediaUseError(
                "This account does not have permission to use the media attached to this message."
            )
...
def check_media_consistency(sender, instance, **kwargs):
    if instance.media_item:
        if not (
            ...
            and instance.media_type == ContentType.objects.get_for_model(Photo)
        ) and not (
            (
                instance.message_type == "free_video"
                or instance.message_type == "paid_video"
            )
            and instance.media_type == ContentType.objects.get_for_model(Video)
        ):
            raise MediaUseError(
                "Message type does not match media type. Please, correct the discrepancy."
            )
...
def enforce_chat_initiation_privileges(sender, instance, **kwargs):
    ...
    if not ChatMessage.objects.filter(qlookup).exists():
        if not Subscription.objects.filter(
            subscriber=instance.user, subscribed_to=instance.receiver
        ).exists():
