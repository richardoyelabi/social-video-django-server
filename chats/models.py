...
class Thread(models.Model):
    ...
    first = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="chat_thread_first"
    )
    second = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="chat_thread_second"
    )
...
class InboxManager(models.Manager):
    def set_inbox(self, user, other_user, msg, read=False):
        ...
        async_to_sync(channel_layer.group_send)(
            f"inbox_{inbox_object.user.public_id}",
            {"type": "update_inbox", "content": ib1},
        )
    ...
    def read_inbox(self, user, other_user):
        ...
        if inbox.count() == 1:
            ...
            async_to_sync(channel_layer.group_send)(
                f"inbox_{inbox_obj.user.public_id}",
                {
                    "type": "update_inbox",
                    "content": {"type": "read_update", "data": inbox_data},
                },
            )
...
class Inbox(models.Model):
    ...
    second = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="inbox_second_user"
    )
    updated = models.DateTimeField(auto_now=True)
    timestamp = models.DateTimeField(auto_now_add=True)
    last_message = models.TextField(max_length=2048, blank=True, default="")
    last_message_from = models.ForeignKey(
        User,
