...
def check_media_ownership(sender, instance, **kwargs):
    if instance.media_item.uploader != instance.uploader:
        raise MediaUseError(
            "This account does not have permission to use the media in this post."
        )
...
def check_media_consistency(sender, instance, **kwargs):
    if not (
        ...
        raise MediaUseError(
            "Post type does not match media type. Please, correct the discrepancy."
        )
...
def record_unique_view(sender, instance, created, **kwargs):
    ...
    if created:
        if not UniqueView.objects.filter(
            account=instance.account, post=instance.post
        ).exists():
            UniqueView.objects.create(
                account=instance.account, post=instance.post, time=instance.time
            )
