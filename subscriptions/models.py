...
class SubscriptionTransaction(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        self.fee_currency, self.fee_amount = (
            creator_info.subscription_fee_currency,
            creator_info.subscription_fee_amount,
        )
        ...
        for currency in currencies:
            ...
            self.fee_amount = convert_currency(
                source=source_currency, target=currency, amount=source_amount
            )
...
class Subscription(models.Model):
    subscribed_to = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        related_name="subs_subscribed_to_set",
        on_delete=models.CASCADE,
    )
    subscriber = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        related_name="sub_subscriber_set",
        on_delete=models.CASCADE,
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if not (creator_account.is_creator):
            raise SubscriptionNotACreatorError(
                f"{creator_account.username} is not a creator. Can not subscribe to non-creator account."
            )
        if not (creator_account.creatorinfo.is_verified):
            raise SubscriptionNotACreatorError(
                f"{creator_account.username} is not a verified creator."
            )

        try:
            SubscriptionTransaction.objects.create(
                subscribed_to=self.subscribed_to, subscriber=self.subscriber
            )
        except TransactionInsufficientBalanceError:
            raise
        ...
        if not PeriodicTask.objects.filter(
            ...
            schedule, created = IntervalSchedule.objects.get_or_create(
                every=validity_period, period=IntervalSchedule.DAYS
            )
...
class CancelledSubscription(models.Model):
    subscribed_to = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        related_name="cancelled_sub_subscribed_to_set",
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
    )
    subscriber = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        related_name="cancelled_sub_subscriber_set",
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
    )
...
class NullifiedSubscription(models.Model):
    ...
    subscribed_to = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        related_name="nullified_sub_subscribed_to_set",
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
    )
    subscriber = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        related_name="nullified_sub_subscriber_set",
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
    )
