...
class UserPublicProfileSerializer(UserDetailsSerializer):
    ...
    cover_photo = CustomImageFieldSerializer(
        sizes="cover_photo", allow_null=True, read_only=True
    )
...
class CreatorPublicInfoSerializer(serializers.ModelSerializer):
    ...
    def get_usd_subscription_fee(self, obj):
        return str(
            convert_currency(
                obj.subscription_fee_currency, "usd", obj.subscription_fee_amount
            )
        )
    ...
    def get_btc_subscription_fee(self, obj):
        return str(
            convert_currency(
                obj.subscription_fee_currency, "btc", obj.subscription_fee_amount
            )
        )
